(function ($) {
    ...
    $.fn.safeRemove = function () {
        ...
        return this.each(function () {
            ...
            // if we have leading or trailing whitespace, replace ourselves with whitespace rather than removing ourselves
            if (text.trim() !== text) {

                $this.replaceWith($.parseHTML(" "));
    ...
    $.fn.cleanMarkup = function (options) {
        ...
        return this.each(function () {
            ...
            p.find(".MsoCommentReference").safeRemove();            // references to footnotes
            p.find(".msocomanchor").safeRemove();                   // footnote references inside hyperlinks
            p.find(".msocomtxt").parent().parent().safeRemove();    // entire footnote block
            ...
            p.find("*").filter(function () {
                ...
                return $this.css("color") == $this.css("background-color");
            ...
            while (children.length === 1 && children.css("display") == "block") {

                children.replaceWith(children.contents());
                children = p.children();
            ...
            p.find("*").contents().filter(textNodeFilter).filter(function () {
                ...
                var re = /\b([a-z0-9._-]+@[a-z0-9._-]+\.[a-z0-9._-]+)\b/ig;
            ...
            // remove blocks with nothing inside
    ...
    $(function () {
        ...
        $(".clean-html-convert").click(function () {

            var $button = $(this);
            var $source = $button.parents().find(".clean-html-source").first();
            var $output = $button.parents().find(".clean-html-output").first();

            if (!$source.length || !$output.length) {

                console.log("Error: unable to proceed without source (.clean-html-source) and output (.clean-html-output) elements");
                return;
            ...
            var options = {};
            var $option1 = $button.parents().find(".clean-html-options").first();
            $option1.parent().find(".clean-html-options").each(function () {

                var $this = $(this);
                var data = $this.data("cleanHtmlOptions");

                if ($this.is(":checked") && data) {
            ...
            // for debugging, having $target in the DOM may be useful, but it's not necessary
            var $target = $button.parents().find(".clean-html-target").first();
            ...
            var html = tidy_html5($target.html(), tidyOptions);
